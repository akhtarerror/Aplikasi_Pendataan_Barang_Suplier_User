...
Public Class Penjualan
    ...
    Dim totalhargapenjualan As Long
    ...
    Private Sub Button3_Click(sender As Object, e As EventArgs) Handles Button3.Click
        ...
        Else
            bayar = TextBox7.Text
            MessageBox.Show("Berhasil mengurangi stok", "oi", MessageBoxButtons.OK, MessageBoxIcon.Warning)
    ...
    Private Sub PD_PrintPage(sender As Object, e As PrintPageEventArgs) Handles PD.PrintPage
        ...
        topMargin += 20
        e.Graphics.DrawString("Penjualan Stok", f10, Brushes.Black, e.PageBounds.Width / 2, topMargin, tengah)
        ...
        hitungfaktur()
        e.Graphics.DrawString(garis, f10, Brushes.Black, leftMargin, topMargin)
        e.Graphics.DrawString("Total : " & Format(totalhargapenjualan, "##,##0"), f10b, Brushes.Black, leftMargin, topMargin + 15)
    ...
    Sub faktur()
        ...
        koneksi()
        cmd = New MySqlCommand("select * from penjualan where left(faktur,6)='" & tgl & "'order by faktur desc", conn)
        rd = cmd.ExecuteReader
        rd.Read()
    ...
    Sub simpan()
        ...
        Try
            ...
            str = "INSERT INTO penjualan (tgl, total, bayar, kembali) VALUES (@tgl, @total, @bayar, @kembali)"
            ...
            For baris As Long = 0 To DataGridView1.RowCount - 1
                Try
                    ...
                    str = "INSERT INTO detailtransaksijual (nama_barang, jumlah, harga_satuan, total, faktur) VALUES (@nama_barang, @jumlah, @harga_satuan, @total, @faktur)"
                    cmd = conn.CreateCommand
                    With cmd
                        .CommandText = str
    ...
    Sub pindah()
        ...
        If jum > stok Then
            ...
            Exit Sub ' Exit the sub if quantity is greater than stock
        End If

        ' Insert the new row at the top
        DataGridView1.AllowUserToAddRows = True
        DataGridView1.Rows.Insert(0, 1)
        DataGridView1(0, 0).Value = kode
        DataGridView1(1, 0).Value = TextBox2.Text
        DataGridView1(2, 0).Value = jum
        DataGridView1(3, 0).Value = har
        DataGridView1(4, 0).Value = tot
        DataGridView1.AllowUserToAddRows = False

        ' Loop through the rows to check for duplicate codes and update quantities
        For barisatas As Long = 1 To DataGridView1.RowCount - 1
            Dim b_atasjumlah As Long
            b_atasjumlah = DataGridView1.Rows(barisatas).Cells(2).Value

            If DataGridView1.Rows(barisatas).Cells(0).Value = kode Then
                Dim totjumlah As Long
                totjumlah = b_atasjumlah + jum

                If totjumlah > stok Then
                    MessageBox.Show("Jumlah lebih besar dari stok yang ada" & vbCrLf _
                             & "Stok hanya " & stok, "Oops", MessageBoxButtons.OK, MessageBoxIcon.Warning)
                    DataGridView1.Rows(barisatas).Cells(2).Value = stok
                    DataGridView1.Rows(barisatas).Cells(3).Value = DataGridView1.Rows(barisatas).Cells(3).Value
                    ...
                    DataGridView1.Rows.RemoveAt(0) ' Remove the top row if quantity exceeds stock
                    Exit Sub
                Else
                    DataGridView1.Rows(barisatas).Cells(2).Value = totjumlah
                    DataGridView1.Rows(barisatas).Cells(3).Value = DataGridView1.Rows(barisatas).Cells(3).Value
                    DataGridView1.Rows(barisatas).Cells(4).Value = (totjumlah * DataGridView1.Rows(barisatas).Cells(3).Value)
                    DataGridView1.Rows.RemoveAt(0) ' Remove the top row as it's a duplicate
                    Exit Sub
                End If
            End If
        Next
